...
func (c *MysqlStorage) Init() error {
	...
	var count int64
	if c.Model(&Policy{}).Count(&count); count == 0 {
...
func (c *MysqlStorage) PolicyReader() ([]Policy, error) {
...
func (c *MysqlStorage) PolicyWriter(policies []Policy) error {
	var err error
	// 生成策略名字
	for i, p := range policies {
		if p.Type != AggRule || p.RuleID != RuleMatch.ID || p.Name != "" {
