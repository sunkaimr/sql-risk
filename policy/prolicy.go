...
func GenerateDefaultPolicy() []Policy {
	policies := []Policy{
		...
		},
		{
			...
			Suggestion:  "注意：影响行数是通过EXPLAIN得到的结果与最终的数据修改量可能会有偏差",
		},
		{
		...
		},
		{
			...
			Special:     false,
			Priority:    70,
			Description: "DELETE或UPDATE操作的影响行数是否小于等于10w",
			Suggestion:  "注意：影响行数是通过EXPLAIN得到的结果与最终的数据修改量可能会有偏差",
		},
		{
		...
		// DELETE
		{
			...
			RuleID:      RuleMatch.ID,
			Operator:    RuleOperatorALL,
			Value:       []string{"OPE.DELETE.002"},
			Level:       comm.Fatal,
			Special:     false,
			Priority:    200,
			Description: "删除全表数据",
			Suggestion:  "删除全表数据建议通过truncate来完成",
		},
		//{
		...
		//},
		{
			...
			RuleID:      RuleMatch.ID,
			Operator:    RuleOperatorALL,
			Value:       []string{"OPE.DELETE.002", "RUN.CAPACITY.003"},
			Level:       comm.Low,
			Special:     false,
			Priority:    200,
			Description: "删除全表数据且表的行数小于等于10w",
			Suggestion:  "",
		},
		{
		...
		},
		{
			...
			Special:     false,
			Priority:    210,
			Description: "根据WHERE条件删除表行数小于等于10w的表",
			Suggestion:  "",
		},
		{
			...
			RuleID:      RuleMatch.ID,
			Operator:    RuleOperatorALL,
			Value:       []string{"OPE.DELETE.001", "RUN.TABINFO.008", "OPE.AFFECTROWS.001"},
			Level:       comm.High,
			Special:     true,
			Priority:    200,
			Description: "根据WHERE条件删除数据，WHERE条件中的各列都没有索引，且影响行数大于10w",
			Suggestion:  "注意：影响行数是通过EXPLAIN得到的结果与最终的数据修改量可能会有偏差",
		},
		{
			...
			RuleID:      RuleMatch.ID,
			Operator:    RuleOperatorALL,
			Value:       []string{"OPE.DELETE.001", "RUN.TABINFO.008", "OPE.AFFECTROWS.005"},
			Level:       comm.High,
			Special:     false,
			Priority:    200,
			Description: "根据WHERE条件删除数据，WHERE条件中的各列都没有索引，且影响行数小于等于10w",
			Suggestion:  "注意：影响行数是通过EXPLAIN得到的结果与最终的数据修改量可能会有偏差",
		},
		{
			...
			RuleID:      RuleMatch.ID,
			Operator:    RuleOperatorALL,
			Value:       []string{"OPE.DELETE.001", "RUN.TABINFO.007", "OPE.AFFECTROWS.001"},
			Level:       comm.High,
			Special:     true,
			Priority:    200,
			Description: "根据WHERE条件删除数据，WHERE条件中某列存在索引，且影响行数大于10w",
			Suggestion:  "注意：影响行数是通过EXPLAIN得到的结果与最终的数据修改量可能会有偏差",
		},
		{
			...
			RuleID:      RuleMatch.ID,
			Operator:    RuleOperatorALL,
			Value:       []string{"OPE.DELETE.001", "RUN.TABINFO.007", "OPE.AFFECTROWS.005"},
			Level:       comm.Low,
			Special:     false,
			Priority:    200,
			Description: "根据WHERE条件删除数据，WHERE条件中某列存在索引，且影响行数小于等于10w",
			Suggestion:  "注意：影响行数是通过EXPLAIN得到的结果与最终的数据修改量可能会有偏差",
		},
		// UPDATE
		{
		...
		},
		{
			...
			RuleID:      RuleMatch.ID,
			Operator:    RuleOperatorALL,
			Value:       []string{"OPE.UPDATE.002", "RUN.CAPACITY.003"},
			Level:       comm.Low,
			Special:     false,
			Priority:    200,
			Description: "更新全表数据且表行数小于等于10w",
			Suggestion:  "",
		},
		{
			...
			Special:     false,
			Priority:    210,
			Description: "根据WHERE条件更新表行数小于等于10w的表",
			Suggestion:  "",
		},
		{
		...
		},
		{
			...
			RuleID:      RuleMatch.ID,
			Operator:    RuleOperatorALL,
			Value:       []string{"OPE.UPDATE.001", "RUN.TABINFO.008", "OPE.AFFECTROWS.001"},
			Level:       comm.High,
			Special:     true,
			Priority:    200,
			Description: "根据WHERE条件更新数据，WHERE条件中的各列都没有索引，且影响行数大于10w",
			Suggestion:  "注意：影响行数是通过EXPLAIN得到的结果与最终的数据修改量可能会有偏差",
		},
		{
			...
			RuleID:      RuleMatch.ID,
			Operator:    RuleOperatorALL,
			Value:       []string{"OPE.UPDATE.001", "RUN.TABINFO.008", "OPE.AFFECTROWS.005"},
			Level:       comm.High,
			Special:     false,
			Priority:    200,
			Description: "根据WHERE条件更新数据，WHERE条件中的各列都没有索引，且影响行数小于等于10w",
			Suggestion:  "注意：影响行数是通过EXPLAIN得到的结果与最终的数据修改量可能会有偏差",
		},
		{
			...
			RuleID:      RuleMatch.ID,
			Operator:    RuleOperatorALL,
			Value:       []string{"OPE.UPDATE.001", "RUN.TABINFO.007", "OPE.AFFECTROWS.001"},
			Level:       comm.High,
			Special:     true,
			Priority:    200,
			Description: "根据WHERE条件更新数据，WHERE条件中某列存在索引，且影响行数大于10w",
			Suggestion:  "注意：影响行数是通过EXPLAIN得到的结果与最终的数据修改量可能会有偏差",
		},
		{
			...
			RuleID:      RuleMatch.ID,
			Operator:    RuleOperatorALL,
			Value:       []string{"OPE.UPDATE.001", "RUN.TABINFO.007", "OPE.AFFECTROWS.005"},
			Level:       comm.Low,
			Special:     false,
			Priority:    200,
			Description: "根据WHERE条件更新数据，WHERE条件中某列存在索引，且影响行数小于等于10w",
			Suggestion:  "注意：影响行数是通过EXPLAIN得到的结果与最终的数据修改量可能会有偏差",
		},
		// CREATE
		{
		...
		{
			PolicyID:    "AGG.RULEMATCH.351",
			Name:        "清空大表",
			Enable:      true,
			Type:        AggRule,
			RuleID:      RuleMatch.ID,
			Operator:    RuleOperatorALL,
			Value:       []string{"OPE.TRUNCATE.001", "RUN.CAPACITY.004"},
			Level:       comm.High,
			Special:     true,
			Priority:    200,
			Description: "清空大表",
		...
		{
			PolicyID:    "AGG.RULEMATCH.352",
			Name:        "清空小表",
			Enable:      true,
			Type:        AggRule,
			RuleID:      RuleMatch.ID,
			Operator:    RuleOperatorALL,
			Value:       []string{"OPE.TRUNCATE.001", "RUN.CAPACITY.003"},
			Level:       comm.Low,
			Special:     false,
			Priority:    200,
			Description: "清空小表",
