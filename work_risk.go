...
func (c *WorkRisk) SplitStatement() error {
	...
	if idx != -1 {
		...
		word := comm.GetSurroundingWords(c.SQLText, idx)
		c.SetItemError(ParseSQL, fmt.Errorf("第%d字符 \"%s\"附近发现中文空格，中文空格可能引发兼容性问题，请避免使用中文空格", idx, word))
		//c.SetMatchPolicies(policy.NewCustomPolicy(ParseSQL, "",
		//	fmt.Sprintf("第%d字符 \"%s\"附近发现中文空格", idx, word),
		//	"中文空格可能引发兼容性问题，请避免使用中文空格",
		//	comm.Info))
	}
